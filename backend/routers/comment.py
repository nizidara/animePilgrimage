...
@limiter.limit("60/minute")  
async def report_comment_detail(request: Request, delete_comment_id: int, db: AsyncSession = Depends(get_db)):
...
@limiter.limit("60/minute")  
async def report_comment_detail(request: Request, db: AsyncSession = Depends(get_db)):
...
async def approve_report_comment(request: Request, delete_comment_id: int, db: AsyncSession = Depends(get_db)):
...
@limiter.limit("10/minute")  
async def delete_comment_report(request: Request, delete_comment_id: int, db: AsyncSession = Depends(get_db)):
